let curTheme='dark', namingMode='sharp', linkedNav=true, labelMode='notes', currentInstrument='piano', guitarSortMode='fretboard';

function toggleGuitarSort(){
  guitarSortMode = guitarSortMode === 'fretboard' ? 'list' : 'fretboard';
  const b = document.getElementById('sortBtn');
  if(b) b.textContent = 'Sort: ' + (guitarSortMode==='fretboard' ? 'Fretboard' : 'List');
  if(cur) {
...
function isMobile(){return window.innerWidth<=768}function initMobileCollapse(){if(!isMobile())return;document.querySelectorAll('.collapse-body-target').forEach(el=>{el.classList.add('collapse-body', 'open');const hdr=el.previousElementSibling;if(hdr&&hdr.classList.contains('collapse-header'))hdr.classList.add('open');})}
...
const DARK_THEMES={dark:1,warm:1,ocean:1,gold:1,mint:1,rose:1,vapor:1,midnight:1,ember:1,mocha:1};
const THEME_VARS={
...
function genShellV(shellDef,rootSemi,bassString,otherStrings,ct){const v=[],ch=CH[ct],roleIvMap={};ch.r.forEach((r,i)=>{roleIvMap[r]=ch.iv[i]});
  ...
  for(const pair of otherStrings){for(const ord of orderings){const midIdx=pair[0],hiIdx=pair[1];const midOpen=GS[midIdx],hiOpen=GS[hiIdx];
    ...
    const best=findSpan(frets, true, 3);if(best){const fv=Array(6).fill(null);best.forEach(f=>{fv[f.stringIdx]={fret:f.fret,deg:f.parentRole,civ:roleIvMap[f.parentRole]||0,finger:0}});
...
function findSpan4(frets){const c=[];const n=frets.length;const octs=new Array(n).fill(0);function recurse(idx){if(idx===n){const adj=frets.map((f,i)=>{let fr=f.fret;if(octs[i]===1)fr+=12;if(octs[i]===2)fr-=12;return{...f,fret:fr}});if(adj.some(f=>f.fret<0||f.fret>19))return;const ft=adj.filter(f=>f.fret>0);const allFrets=adj.map(f=>f.fret);if(allFrets.some(f=>f===0))return;if(!ft.length){c.push({v:adj,span:0,min:0});return}const mn=Math.min(...ft.map(f=>f.fret)),mx=Math.max(...ft.map(f=>f.fret));if(mx-mn<=4)c.push({v:adj,span:mx-mn,min:mn});return}for(let o=0;o<3;o++){octs[idx]=o;recurse(idx+1)}}recurse(0);if(!c.length)return null;c.sort((a,b)=>{return a.span!==b.span?a.span-b.span:a.min-b.min});return c[0].v}
...
v.push({name:DROP2_INV_NAMES[inv],voicing:fv,br:null,triadAbbr:'drop2',triadRootRole:'root',triadRootSemi:rs,drop2Inv:inv,d2Name:d2Name,d2Label:d2Label,btnRole:baseRole,topRole:topRole})}}return v}
  ...
  playableShells.forEach(sh=>{
    ...
    sp.onclick=()=>{
      ...
      if(semis && semis.length){
        ...
        previewVoicingOnPiano(semis, lastRole, rs, CH[ct]);
        semis.forEach((s,i)=>pT(s,now+i*.008,2.5,ac));
      }
    };
    tags.appendChild(sp);
...
function animateGuitarCircle(ci) {
  if(!ci) return;
  const orig = ci.dataset.origR || ci.getAttribute('r');
  if(!ci.dataset.origR) ci.dataset.origR = orig;
  
  // Instantly snap to larger size
  ci.style.transition = 'none';
  ci.setAttribute('r', String(parseFloat(orig) * 1.3));
  
  // Force reflow so the browser registers the jump instantly
  ci.getBoundingClientRect(); 
  
  // Smoothly transition back
  ci.style.transition = 'r 0.3s cubic-bezier(0.25, 1, 0.5, 1)';
  ci.setAttribute('r', orig);
}
    ...
    setTimeout(()=>{
      keys.forEach(k=>{
        if(parseInt(k.dataset.midi)===m){
          ...
          void k.offsetWidth; 
          k.style.transition = '';
          k.classList.add('key-flash');
          if(k.flashTimer) clearTimeout(k.flashTimer);
          k.flashTimer = setTimeout(()=>k.classList.remove('key-flash'),300);
        }
      });
      gNotes.forEach(g=>{
        if(parseInt(g.dataset.midi)%12===m%12){
          animateGuitarCircle(g.querySelector('circle'));
        }
      });
    },d);
  });
...
function sgN(gi,dir){
  setGuitarSrc(gi); // Auto-selects the panel for playback when the arrow is clicked
  if(linkedNav){
  ...
  if(playSrc==='guitar'){stopAll(true);const semis=guitarMidi();if(semis.length){const ac=initAudio(),now=ac.currentTime;semis.forEach((s,i)=>pT(s,now+i*.008,2.5,ac));aK(semis,false);document.getElementById('pdot').classList.add('on');setTimeout(()=>document.getElementById('pdot').classList.remove('on'),2000)}}
}
...
if (currentInstrument === 'piano') {
  document.getElementById('pianoPanel').style.display='block';
  document.getElementById('guitarSection').style.display='none';
} else {
  document.getElementById('pianoPanel').style.display='none';
  document.getElementById('guitarSection').style.display='block';
}
}
...
document.getElementById('guitarSection').style.display='none';
document.getElementById('pianoPanel').style.display='none';
document.querySelectorAll('#pianoKeys .wk, #pianoKeys .bk').forEach(k=>{HL.forEach(cls=>k.classList.remove(cls));const lbl=k.querySelector('.piano-note-label');if(lbl)lbl.style.removeProperty('color')});document.getElementById('qRoot').textContent=spellRoot(ri);
...
function setSrc(s){
  ...
  if(isContinuous()){stopAll();playChord()}
}
function selectAll(){
...
function buildGuitar(ct,rs){
  ...
  document.getElementById('gTabShells').style.display=hasShells?'':'none';
  document.getElementById('gTabDrop2').style.display=hasDrop2?'':'none';
  
  if(guitarDiagMode==='shells'&&!hasShells){
    guitarDiagMode='triads';
    document.getElementById('gTabTriads').classList.add('on');
    document.getElementById('gTabShells').classList.remove('on');
    document.getElementById('gTabDrop2').classList.remove('on');
  }
  if(guitarDiagMode==='drop2'&&!hasDrop2){
    guitarDiagMode='triads';
    document.getElementById('gTabTriads').classList.add('on');
    document.getElementById('gTabShells').classList.remove('on');
    document.getElementById('gTabDrop2').classList.remove('on');
  ...
  if(guitarDiagMode==='shells'){ctd=tempGS;sgi=[0,0]}
  else if(guitarDiagMode==='drop2'){ctd=buildDrop2Groups(ct,rs);sgi=[0,0]}
  else{ctd=buildGT(ct,rs);sgi=[0,0,0]}
  renderSGs()
}
...
const h=document.createElement('div');
    ...
    h.innerHTML=`<span class="string-group-label" id="sgL-${gi}">${strLabel} <span style="font-weight:400;color:var(--txt3);font-size:9px">${_voicingStrNames(v)}</span></span><div class="string-group-nav"><button class="sg-arrow" onclick="sgN(${gi},-1)">‹</button><span class="sg-counter" id="sgC-${gi}">${sgi[gi]+1}/${vc}</span><button class="sg-arrow" onclick="sgN(${gi},1)">›</button></div>`;
    d.appendChild(h);
    const info=document.createElement('div');
    info.className='triad-info';
    info.id='sgI-'+gi;
    info.innerHTML=sgInfoHtml(v,sg);
...
function playSgChord(e){
  ...
  semis.forEach((s,i)=>pT(s,now+i*.008,2.5,ac));
  aK(semis,false);
  previewVoicingOnPiano(semis,v.voicing.find(x=>x)?.deg||'root',cur.rootSemi,cur.ch);
  document.getElementById('pdot').classList.add('on');
  setTimeout(()=>document.getElementById('pdot').classList.remove('on'),2000);
}
...
function drawSG(gi){
  ...
  let col = (DEG_COLORS[fullV.btnRole]||DEG_COLORS['root']).bg;
...
function getShellNotes(ct){
  ...
  const sevIv=has7?ch.iv[ch.r.findIndex(r=>r.includes('7'))]:(ch.r.includes('13th')?ch.iv[ch.r.indexOf('13th')]:null);
  const sevRole=has7?ch.r.find(r=>r.includes('7')):'13th';
  ...
  // Generate Rootless Superimposed Shells
  const r3=thirdRole, r7=has7?ch.r.find(r=>r.includes('7')):null;
